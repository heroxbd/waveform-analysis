...
The PE, waveform and their estimators are hierarchical and intercorellated.  We explicitly summarize the symbol conventions of this article in table~\ref{tab:symbol}.

\begin{table}[!ht]
  \centering
  \caption{random variables}
  \begin{tabular}{ccccc}
    variable & estimator & related & type & meaning \\
    \hline
    $\mu$ & $\hat{\mu}$ & $\hat{\mu}_\mathrm{KL}, \hat{\mu}_Q$ & random & light intensity \\
    $t_0$ & $\hat{t}$ & $\hat{t}_\mathrm{ALL}, \hat{t}_\mathrm{KL}, \hat{t}_\mathrm{1st}, \Delta{t_0}$ & random & time shift of the light curve \\
    $\phi(\cdot)$ & $\hat{\phi}$ & $\tilde{\phi}, \hat{\phi}_*, \hat{\Phi}, \tilde{\phi}_*, \tilde{\Phi}$ & constant & normalized light curve \\
    $(\sigma_\ell, \tau_\ell)$ & & & constant & light curve shape parameters \\
    $V_\mathrm{PE}(\cdot)$ & & $V_{\mathrm{PE}*}$& constant & waveform pulse of a single PE \\
    ...
    $w(\cdot)$ & $\hat{w}$ & $\bm{w}, w'$ & random & PMT waveform \\
    $\tilde{w}$ & & $\tilde{w}_*$ & random & smoothed $w$ \\
    $\epsilon(\cdot)$ & & & random & white noise \\
    $\sigma^2_\epsilon$ & & & constant & variance of the white noise \\
    $(t_i, q_i)$ & $(\hat{t}_i, \hat{q}_i)$ & $\bm{\hat{t}}, \hat{\bm{q}}$ & random & location and amplitude of the i-th PE \\
    $\sigma_\mathrm{1st}$ & & & & $\sqrt{\Var[\hat{t}_\mathrm{1st}]}$ \\
    $N_\mathrm{PE}$ & $\hat{N}_\mathrm{PE}$ & & random & number of PEs \\
    ...
    $t'_i$ &&& constant & grid of PE location candidates \\
    $q'_i$ &&& random & charge of PE at location $t'_i$ \\
    $N_s$ &&& constant & number of grid locations
  \end{tabular}
  \label{tab:symbol}
\end{table}
