...
\begin{itemize}
    ...
    \item $q_{r}(t)$ - Reconstructed charge
    \item $n_{r}(t)$ - Reconstructed number of PE (\#PE)
...
\end{itemize}
The $t$ we defined here is discrete value in 1 DAQ window (from 0 to 1029ns, step size is 1ns in Jinping 1ton prototype DAQ system). Therefore $N_{pos}$ is not equal to $N_{pe}$ ($N_{pe} \geq N_{pos}$). 
...
Most of the time, when handling PMT waveform we record the first $t$ of PE arrival according to $v_{th}$ and $Q$. One waveform is converted to a pair of numbers. More detailed information of the waveform was lost. The intention this work is extracting information of all hits in 1 DAQ window including $t$ \& $q_{r}(t)$. 
