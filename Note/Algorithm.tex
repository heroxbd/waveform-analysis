...
Third method is Fourier deconvolution. First we extract enough single-photoelectron (SPE) response, $v_{spe}(t)$, from simulation data in fitting method. We suppose the average of these $v_{w}$ is the $v_{spe}$. Assuming that the waveform is the convolution of $v_{spe}$ and $q_{rec}(t)$, the Fast Fourier transform (FFT) of $v_{w}$ and $v_{spe}$ are computed. A rectangular filter, $f$, is added. Then compute the inverse FFT of the quotient, which is between filtered FFT of $v_{w}$ and FFT of $v_{spe}$. 

\begin{itemize}
    \item $q_{rec}(t) = \mathcal{F}^{-1}\{\frac{f[\mathcal{F}(v_{w})]}{\mathcal{F}(v_{spe})}\}$
\end{itemize}
...
Lucy-Richardson deconvolution is a non linear iteration method, to calculate the deconvolution of signals. Lucy deconvolution has an advantage against Fourier deconvolution which is a constraint that $q_{rec}(t)$ are larger than 0. 
...
\subsection{Fitting}
Waveform is the convolution of $v_{spe}$ and $q_{rec}(t)$, using $q_{rec}(t)$ as an example below (see formula~\eqref{eq:wave-con}). 
\begin{align}
...
In fitting process, the parameter is $q_{rec}(t)$ in each discrete $t_{w}$. The complete information hided in origin waveform is disturbed by background noise and pile-up. The loss (see formula~\ref{eq:loss-rss}) which is optimized is the Residual sum of squares (RSS) between the origin wave $v_{w}(t)$ and reconstructed wave $v_{r}(t)$ according to the parameters. The parameters will tend to be preciser when the reconstructed wave approaching to the origin wave. 

Fitting program is implemented with Limited-memory BFGS with bound constraint\cite{byrd_limited_1995} (L-BFGS-B). The $q_{rec}$ and $n_{r}$ are constrained to be larger than 0. 
