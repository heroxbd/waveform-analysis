...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
This $\hat{\alpha}$ scaling process enables us to eliminate noise. Figure~\ref{fig:fd} illustrates the algorithm with an example having $D_w = \SI{2.03}{ns}$. 
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{MCMC results when $\mu=4, \tau_l=\SI{20}{ns}, \sigma_l=\SI{5}{ns}$. A demonstration shows in~\subref{fig:mcmc}.}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\label{fig:fbmp-npe} $D_w$ vs $N_{\mathrm{PE}}$ Box plot \& Histogram, $\mu=4, \tau_l=\SI{20}{ns}, \sigma_l=\SI{5}{ns}$, method is FBMP}
\end{figure}
