...
We choose a shallow network structure of 5 layers to recognize patterns as shown in figure~\ref{fig:struct}, motivated by the pulse shape and universality of $V_\mathrm{PE}(t)$ for all the PEs. The input vector is convoluted by several kernels $K^{(1)}_m$ into several new vectors $v_m$:
\begin{equation}
  \bm{v}^{(1)}_m = K^{(1)}_m \otimes \bm{w}
  \label{eq:1DCNN-11}
\end{equation}
After this linear operation called ``links'', a point-wise nonlinear transformation called ``activation'' is applied. In this study leaky relu function $R$ is used:
\begin{equation}
  \begin{aligned}
    & \bm{v'}^{(1)}_m = \mathrm{leaky\_relu}(\bm{v}^{(1)}_m) \\
    \text{where  } & \mathrm{leaky\_relu}(x) = \left\{ \begin{aligned}
      & 0.05 x & x<0 \\
      & x & x\geqslant 0 \\
    \end{aligned} \right.
  \end{aligned}
...
\begin{equation}
  \begin{aligned}
    & \bm{v}^{(2)}_m = \sum_{m} K^{(2)}_{nm} \otimes \bm{v'}^{(1)}_m \\
    & \bm{v'}^{(2)}_m = \mathrm{leaky\_relu}(\bm{v}^{(2)}_m) \\
  \end{aligned}
  \label{eq:1DCNN-2}
\end{equation}

The convolutional widths are selected considering the localized nature of $V_\mathrm{PE}(t)$.  
