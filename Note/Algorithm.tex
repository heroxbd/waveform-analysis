...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{\label{fig:method}Demonstrations of heuristic methods on a waveform sampled from $\mu=4$, $\tau_\mathrm{l}=\SI{20}{ns}$, $\sigma_\mathrm{l}=\SI{5}{ns}$ light curve conditions.  Peak finding in~\subref{fig:peak} handles charges more realistically than waveform shifting in~\subref{fig:shifting}, giving better numbers by the $RSS$ and $D_w$ criteria in section \ref{sec:criteria}.}
\end{figure}
...
In figure~\ref{fig:cnn-npe}, we use box plot to describe $D_w$ distributions. The middle line is the median of distribution. The upper and lower hinge are first quartile~($Q_1$, 25th percentile) and third quartile~($Q_3$, 75th percentile). The upper and lower whiskers are $Q_1-1.5(Q_3-Q_1)$ and $Q_3+1.5(Q_3-Q_1)$, respectively.  This definition applies throughout this paper.  

It is reasonable that $D_w$ is the smallest for one PE.  $D_w$ stops increasing with $N_\mathrm{PE}$ at about 6 PEs, showing CNN performs well in $D_w$ at large pile-ups.  The small $D_w$ from CNN provides a precise matching of waveforms horizontally to guarantee effective $\hat{\alpha}$ scaling, explaining why RSS is also small in figure~\ref{fig:cnn}.
...
\begin{figure}[H]
    ...
    \caption{\label{fig:fbmp-npe} $D_w$ histogram and its distributions of FBMP on $\num[retain-unity-mantissa=false]{1e4}$ waveforms sampled from the same setup as figure~\ref{fig:method}, conditioned on $N_{\mathrm{PE}}$.  The horizontal axis of the right subplot is in arbitrary unit. }
\end{figure}
