...
We replace $\hat{N}$ with a fixed sample size $N_s$ and $\hat{t}_i$ with a fixed vector of times $t'_i$ in eq.~\eqref{eq:w-hat}, 
\begin{equation}
  \label{eq:gd}
...
\end{equation}
From the output $\hat{\phi}_\mathrm{dec}(t)$ of a deconvolution method in section~\ref{sec:lucyddm}, we can confidently leave out all the $t'_i$ that $\hat{\phi}_\mathrm{dec}(t_i')=0$ in eq.~\eqref{eq:gd-phi} to reduce the number of parameters.
...
We optimize eq.~\eqref{eq:gd-q} with constraint $q'_i \ge 0$ by Broyden-Fletcher-Goldfarb-Shanno algorithm with a bound constraint~\cite{byrd_limited_1995}. To However, because this method minimizes RSS, we do not scale the converged $\bm{\hat{q}}$. The demonstration~\ref{fig:fitting} here shows a result with $D_w = \SI{0.68}{ns}$.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Gradient descent results when $\mu=4, \tau_\mathrm{l}=\SI{20}{ns}, \sigma_\mathrm{l}=\SI{5}{ns}$ show in~\subref{fig:fitting-npe}. The horizontal axis of the right subplot is in arbitrary unit. A demonstration shows in~\subref{fig:fitting}.}
\end{figure}

The result of $\hat{q}$ when $\mu=4, \tau_\mathrm{l}=\SI{20}{ns}$, and $\sigma_\mathrm{l}=\SI{5}{ns}$ is shown in figure~\ref{fig:fitting-npe}. The left panel is the box plot which shows the $D_w$ percentile vs $N_{\mathrm{PE}}$, and the right panel is the overall histogram of $D_w$. 
...
$D_w$ with $\mu=4, \tau_\mathrm{l}=\SI{20}{ns}$, and $\sigma_\mathrm{l}=\SI{5}{ns}$ is shown in figure~\ref{fig:mcmc-npe}.
\begin{figure}[H]
  \begin{subfigure}{.5\textwidth}
    \centering
  ...
  \end{subfigure}
  \caption{MCMC results when $\mu=4, \tau_\mathrm{l}=\SI{20}{ns}, \sigma_\mathrm{l}=\SI{5}{ns}$ show in~\subref{fig:mcmc-npe}. The horizontal axis of the right subplot is in arbitrary unit. A demonstration shows in~\subref{fig:mcmc}.}
\end{figure}
