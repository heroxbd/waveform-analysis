...
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/pepos.pgf}}
...
\begin{table}[H]
    ...
    \begin{tabular}{r|r|r|r}
        \hline
        EventID & ChannelID & HitTime & Charge \\ \hline
        1       & 4         & 271     & 256.23 \\ \hline
        1       & 4         & 272     & 70.78  \\ \hline
        1       & 4         & 276     & 150.14 \\ \hline
        1       & 4         & 278     & 148.26 \\ \hline
        1       & 4         & 283     & 262.02 \\ \hline
        1       & 4         & 284     & 90.74  \\ \hline
        1       & 4         & 285     & 166.08 \\ \hline
        1       & 4         & 287     & 90.57  \\ \hline
        1       & 4         & 290     & 52.15  \\ \hline
        1       & 4         & 290     & 102.82 \\ \hline
        1       & 4         & 293     & 224.37 \\ \hline
        1       & 4         & 301     & 78.9   \\ \hline
        1       & 4	        & 306	  & 138.40 \\ \hline
    \end{tabular}
    \end{adjustbox}
...
Third method is Fourier deconvolution. First we extract enough $v_{spe}(t)$ from simulation data in fitting method. We suppose the average of these $v_{w}$ is the $v_{spe}$. Assuming that the waveform is the convolution of $v_{spe}$ and $q_{r}(t)$ (or $n_{r}(t)$), the Fast Fourier transform (fft) of $v_{w}$ and $v_{spe}$ are computed. A rectangular filter is added. Then compute the inverse fft of the quotient, which is between filtered fft of $v_{w}$ and fft of $v_{spe}$. 
...
Lucy-Richardson deconvolution is a non linear iteration method, to calculate the deconvolution of signals. Lucy deconvolution has an advantage against Fourier deconvolution which is a constraint that $q_{r}(t)$ or $n_{r}(t)$ are larger than 0. 
...
\subsection{Fitting}
Waveform is the convolution of $v_{spe}$ and $q_{r}(t)$ (or $n_{r}(t)$), using $q_{r}(t)$ as an example below (see formula~\eqref{eq:wave-con}). 
\begin{align}
...
The result reconstructing $q_{r}$ when $N_{pos}=4$ shows in figure. (see figure~\ref{fig:fitting-hist}) The overall result of W-dist reconstructing $q_{r}$ shows in figure (see figure~\ref{fig:fitting-npos}). The error bar shows 10 to 90 percentile of Wasserstein distance distribution. 
