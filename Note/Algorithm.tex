...
The first intuitive method is the find peak method. We first smooth the input waveform using Savitzky–Golay filter to alleviate the obstruction of electron noise. Then we find the peak of the smoothed waveform and move the peaks leftward. The shifted voltage is now promotional to reconstructed charge $\hat{q}(t)$ related with the corresponding hittime. The leftward distance we moved, $\Delta t$ is the peak time of SPE response $V_\mathrm{PE}(t)$, described by formula \eqref{eq:dayaspe}. Finally, the charge is normalized to retain the consistency with the total charge which is the integration of the waveform. The demonstration here shows a result with $D_w = \SI{1.96}{ns}$. 

\begin{figure}[H]
  \begin{subfigure}{0.5\textwidth}
    \centering
    \scalebox{0.4}{\input{figures/findpeak.pgf}}
    \caption{Peak finding: $D_w = \SI{1.96}{ns}$}
  \end{subfigure}
  \begin{subfigure}{0.5\textwidth}
    \centering
    \scalebox{0.4}{\input{figures/threshold.pgf}}
    \caption{Waveform shifting: $D_w = \SI{5.58}{ns}$}
  \end{subfigure}
...
A threshold $V_\mathrm{th}$ is settled with respect to electronics noise in simulation data. In waveform shift method, first we locate waveform voltage, $w(t)$, which exceeds $V_\mathrm{th}$. Then we shift the exceeding part leftward. The $\Delta t$ we move is the same as the find peak method in section \ref{sec:findpeak}. Finally, we treat the normalized exceeding part as $\hat{q}(t)$, to maintain the charge integral of waveforms. The demonstration here shows a result with $D_w = \SI{5.58}{ns}$.

\subsection{Deconvolution}
\subsubsection{Fourier Deconvolution}

Third method is Fourier transform deconvolution. Assuming that the waveform is the convolution of $V_\mathrm{PE}(t)$ and $q(t)$, the Fast Fourier transform (FFT) of $w(t)$ and $V_\mathrm{PE}(t)$ are computed. A rectangular filter, $f$, is implemented. Then we compute the inverse FFT of the quotient between filtered FFT of $w(t)$ and FFT of $V_\mathrm{PE}(t)$. We pick out positive values from inverse FFT result, and normalize them to obtain $\hat{q}(t)$. The process is shown in formula \eqref{eq:fft}. 

\begin{equation}
    \hat{q}(t) = \mathcal{F}^{-1}\{\frac{f[\mathcal{F}(w(t))]}{\mathcal{F}(V_\mathrm{PE}(t))}\}
    \label{eq:fft}
\end{equation}
...
The result reconstructing $\hat{q}$ when $\mu=5, \tau=20\mathrm{ns}, \sigma=10\mathrm{ns}$ shows in figure (see figure~\ref{fig:cnn-hist}). The overall result of $D_w$ reconstructing $\hat{q}$ shows in figure (see figure~\ref{fig:cnn-npe}). 
...
The result reconstructing $\hat{q}$ when $\mu=5, \tau=\SI{20}{ns}, \sigma=\SI{10}{ns}$ shows in figure. (see figure~\ref{fig:fitting-hist}) The overall result of $D_w$ reconstructing $\hat{q}$ shows in the figure (see figure~\ref{fig:fitting-npe}). 
...
We can see during MCMC sampling, the charge w.r.t. time, $\hat{q}$, is also collected. The result reconstructing $\hat{q}$ when $\mu=5, \tau=20\mathrm{ns}, \sigma=10\mathrm{ns}$ shows in figure (see figure~\ref{fig:mcmcrec-chargehist}). 
...
\begin{align}
    w(t) &= F_{w}(q) = \boldsymbol{A}\cdot \boldsymbol{q} \label{eq:fbmpmodel} \\
...
Figure~\ref{fig:fbmp} shows one of the FBMP results. We can see the origin waveform $w(t)$ and reconstructed waveform $v_{r}$ are very similar. The reconstructed and truth $\hat{q}$ are also similar. 
...
The result of reconstructed $\hat{q}$ when $\mu=5, \tau=\SI{20}{ns}, \sigma=\SI{10}{ns}$ are shown in Fig.~\ref{fig:fbmp-hist}. The overall result of $D_w$ reconstructing $\hat{q}$ shows in Fig.~\ref{fig:fbmp-npe}). 
