...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
which reduces to an MLE like eq.~\eqref{eq:2} if $\hat{q}_i\equiv 1$.  $\hat{t}_\mathrm{KL}$ estimates $t_0$ when $t_i, q_i, N_\mathrm{PE}$ are all uncertain.  Similar to $\sigma_\mathrm{1st}$ and $\sigma_\mathrm{ALL}$, the standard deviation $\sigma_\mathrm{KL}$ of $\hat{t}_\mathrm{KL} - t_0$ for a batch of waveforms is the resolution of an algorithm.
...
\begin{figure}[H]
  ...
  \caption{\label{fig:l2} REPLACE THIS FIGURE!  The RSS of red and blue curves is a function of the two shaded regions. It is a constant when the curves shifts horizontally provided they donot overlap.  In contract, the Wasserstein distance of the two curves is sensitive to the distance between them.}
\end{figure}
