...
\end{equation}
After shifting, this method gives $\hat{q}_i = w(t_i)$, $\hat{t_i} = t_i - \Delta t$. $\hat{q}_i$ is multiplied by $\alpha$ to minimize RSS: 

\begin{equation}
  \alpha = \arg\underset{\alpha'}{\min}~\mathrm{RSS}\left[ \alpha' \hat{w}(t), w(t) \right] .
  \label{eq:waveform-shift-alpha}
\end{equation}
The final analysis output is $\hat{q}_i = \alpha w(t_i)$, $\hat{t_i} = t_i - \Delta t$.

Since the whole over-threshold waveform sample points are treated as PEs, one PE can be split into many. Thus, the obtained $\hat{q}_i$ are smaller than PE charges as in figure~\ref{fig:shifting}. Nevertheless, waveform shifting does minimal analysis and serves as a baseline method.
...
\textit{Peak finding} is more effective than waveform shifting by exploiting PE pulses.  We smooth the waveforms by a low-pass Savitzky-Golay filter~\cite{savitzky_smoothing_1964} and find all the peaks at $t_i$.  The following resembles waveform shifting: apply a constant shift $\Delta t$ to get $\hat{t_i} = t_i - \Delta t$, and calculate a scaling factor $\alpha$ to get $\hat{q_i}=\alpha w(t_i)$.  Similar to eq.~\eqref{eq:waveform-shift-dt},

\begin{equation}
  \Delta t \equiv \arg\underset{\Delta t'}{\min} D_\mathrm{w}\left[ \delta(t-\arg\underset{t'}{\max} V_\mathrm{PE}(t')) , \delta(t-\Delta t') \right] = \arg\underset{t}{\max} V_\mathrm{PE}(t)
  \label{eq:peak-finding-dt}
\end{equation}
$\alpha$ is determined the same way as eq.~\eqref{eq:waveform-shift-alpha}. As shown in figure~\ref{fig:peak}, peak finding outputs charges close to 1 and works well for lower PE counts.  But when PEs pile up closely, peaks overlap intensively, making this method unreliable.
...
\begin{equation*}
  ...
  \end{aligned}
\end{equation*}
$\tilde{w}(t)$ unknown in data analysis, we replace it in eq.~(\eqref{eq:rss}) with $w(t)$.
...
\end{equation}
RSS and $D_\mathrm{w}$ are then calculated according to eqs.~(\eqref{eq:rss}) and (\eqref{eq:numerical}).
