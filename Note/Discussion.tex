...
In the fluid-based neutrino and dark matter experiments, hit mode for $\mu < 3$ is the sweet spot for \si{MeV} and \si{keV} physics respectively.  As demonstrated in figure~\ref{fig:deltamu}, FSMP boosts intensity resolution $\sigma_\mu$ by $\sigma_q$, \SIrange{2}{4}{\percent} depending on PMT models. Good waveform analysis has the potential to accelerate discovery and broaden the physics reach of existing detectors. 