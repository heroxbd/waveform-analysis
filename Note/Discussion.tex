...
Figure~\ref{fig:biasmu} shows the bias of $\hat{\mu}$ of all four methods. The bias of LucyDDM comes from the thresholding and scaling defined in eq.~\eqref{eq:fdconv2}. We suspect that the bias of FSMP comes from poisson essence of FSMP. 

Figure~\ref{fig:deltamu} shows the intensity relative resolutions.  For small $\mu$ FSMP gives a much better resolution by correctly modeling charge distributions, see also figure~\ref{fig:recchargehist}.  For larger $\mu$, these four methods are similar to $\hat{\mu}_Q$.
