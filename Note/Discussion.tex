...
Next, we discuss the inference of light intensity.  As we described in \ref{sec:time-shift-t_0}, the waveform samples all have at least 1 PE, though the photoelectric effect on PMT photocathode is a Poisson process. So we complete the intensity estimators here~($\hat{\mu}$) by certain numbers of 0 to make an ideal $\mu$ estimator follows Poisson distributions.  For example, when $\mu=0.5$, we will add 15414 zeros to the $\hat{\mu}$ sample.  The integration estimator $\hat{\mu}_Q$~(``int'' in figures~\ref{fig:biasmu} and~\ref{fig:deltamu}) is ubiquitous.  But as pointed out in section~\ref{sec:intensity-mu}, it is deteriorated by charge smearing $\sigma_q^2$ and white noise.
