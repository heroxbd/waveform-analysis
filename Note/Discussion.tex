...
The hit mode for $\mu < 3$ is the sweet spot for \si{MeV} and \si{keV} in fluid-based neutrino and dark matter experiments.  Demonstrated in figure~\ref{fig:deltamu}, FSMP boosts intensity resolution up to $\sigma_q$, \SIrange{20}{40}{\percent} depending on PMT models, which converts directly to energy resolution in event reconstruction.  It is profound to notice that good analysis having the potential to boost energy resolution, shorten the time to achieve discovery and broaden the physics reach of existing detectors. 
