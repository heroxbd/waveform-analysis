...
def fbmp_inference(a0, a1):
    ...
    for i in range(a0, a1):
        ...
        cs, c_star, psy_star, nu_star, T_star, d_tot_i, d_max_i, xmmse, xmmse_star, num_i = mcp.fbmpr_poisson_deactive(wave_r, A, la, spe_pre[cid]['std'] ** 2, (gsigma * factor / gmu) ** 2, factor, D=np.min([2*int(mu_t+3*np.sqrt(mu_t)),3*A.shape[1]]), Q=np.max([10,maxQ]), stop=5,pmax=2*int(mu_t+3*np.sqrt(mu_t)), left=left_wave, right=right_wave, tlist=tlist,prior=prior)
        ...
        assert abs(nu - nu_star).max() < 1e-6, 'eid:{}'.format(i)
        nu_max[i - a0] = nu[0]
