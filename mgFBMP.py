...
std = 1.
Thres = {'mcmc':std / gsigma, 'lucyddm':0.1, 'fbmp':1e-6, 'poissonfbmp':1e-6}
mix0sigma = 1e-3
...
def fbmp_inference(a0, a1):
    ...
    for i in range(a0, a1):
        ...
        xmmse, xmmse_star, psy_star, nu_star, T_star, d_tot_i, d_max_i = mcp.fbmpr_poisson_reduced(wave_r, A, la, spe_pre[cid]['std'] ** 2, (gsigma * factor / gmu) ** 2, factor, int(D * mu_t), stop=5, truth=truth, i=i, left=left_wave, right=right_wave, tlist=tlist, gmu=gmu, para=p)#wff.fbmpr_fxn_reduced(wave_r, A, la, spe_pre[cid]['std'] ** 2, (gsigma * factor / gmu) ** 2, factor, D, stop=2)
