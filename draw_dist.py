...
if method == 'fbmp':
    ...
    ax2 = fig.add_subplot(gs[1, 0])
    ax2.hist(time['tscharge'][:, 1] - start['T0'], bins=100, label=r'$t_{charge} - t_{0}$')
    ax2.set_xlabel(r'$t_{charge} - t_{0}/\mathrm{ns}$')
    ax2.set_ylabel(r'$Count$')
    ax2.set_yscale('log')
    ax2.legend()
    s = np.std(time['tscharge'][:, 1] - start['T0'], ddof=-1)
...
for i in range(l):
    if i+1 in penum:
        ...
        ax0 = fig.add_subplot(gs[0, 0])
        ax0.hist(dtwpi, bins=np.arange(dtwpi.min(), dtwpi.max()+0.05+1e-6, 0.05))
        ax0.set_xlabel(r'$W-dist/\mathrm{ns}$')
        ax1 = fig.add_subplot(gs[0, 1])
        ax1.hist(rss, bins=np.arange(rss.min(), rss.max()+5+1e-6, 5))
