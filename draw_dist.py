...
for i in range(l):
    if i+1 in penum:
        ...
        ax0 = fig.add_subplot(gs[0, 0])
        ax0.hist(dtwpi, bins=np.arange(dtwpi.min(), dtwpi.max()+0.05, 0.05))
        ax0.set_xlabel(r'$W-dist/\mathrm{ns}$')
        ax1 = fig.add_subplot(gs[0, 1])
        ax1.hist(rss, bins=np.arange(rss.min(), rss.max()+50, 50))
        ax1.set_xlabel(r'$RSS/\mathrm{mV}^{2}$')
        ax2 = fig.add_subplot(gs[1, 0])
        ax2.hist(charged, bins=np.arange(charged.min(), charged.max()+2, 2))
        ax2.set_xlabel(r'$Charge-diff/\mathrm{mV}\cdot\mathrm{ns}$')
        ax3 = fig.add_subplot(gs[1, 1])
        ax3.hist(deltarss, bins=np.arange(deltarss.min(), deltarss.max()+50, 50))
