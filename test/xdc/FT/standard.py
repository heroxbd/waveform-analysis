...
def generate_standard(h5_path, single_pe_path):
    ...
    length = zf0['Waveform']['Waveform'].shape[1]
    npdt = np.dtype([('TrainSet', np.uint8), ('EventID', np.uint32), ('ChannelID', np.uint8), ('Waveform', np.uint16, length), ('speWf', np.uint16, 120)]) # set datatype
    ...
    for i in range(len(h5_path)):
        ...
        with h5py.File(h5_path[i], 'r', libver='latest', swmr=True) as ztrfile:
            ...
            for j in range(l):
                ...
                if (unipe[-1]-unipe[-2]) > 30 and c[-1] == 1 and unipe[-1]<900:
                    single_wf = wf[j]['Waveform'] # temporarily record waveform
                    dt['speWf'][num] = single_wf[(int(unipe[-1]) - 1 - 20) : (int(unipe[-1]) - 1 + 100)] # only record 120 position, and the time of spe is the 21th
                    dt['TrainSet'][num] = i
                    dt['EventID'][num] = eid
                    dt['ChannelID'][num] = ch
                ...
                if np.size(unipe) == 1 and c[0] == 1: # if single pe
                    if unipe[0] < 21 or unipe[0] > 930:
                        ...
                        single_wf = wf[j]['Waveform'] # temporarily record waveform
                        dt['speWf'][num] = single_wf[int(unipe[0]) - 1 - 20 : int(unipe[0]) - 1 + 100] # only record 120 position, and the time of spe is the 21th
                        dt['TrainSet'][num] = i
                        dt['EventID'][num] = eid
                        dt['ChannelID'][num] = ch
                        dt['Waveform'][num] = single_wf
