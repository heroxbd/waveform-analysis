import os
import sys
sys.path.append('./test/CrowdAI_Grader')
import io
import numpy as np
import h5py
import pandas as pd
...
if __name__ == "__main__":
    ...
    psr.add_argument('-tr', dest='train', help='training h5file')
    args = psr.parse_args()

    with h5py.File(args.ref) as ref, h5py.File(args.ipt) as ipt:
        df_ans = ref["GroundTruth"][...]
        df_sub = ipt["Answer"][...]
    wdist, pdist = wpdistance(dt_ans,df_sub)
    with open(args.rec, 'w+') as csvf:
        csvwr = csv.writer(csvf)
        csvwr.writerow([args.train, args.ipt, str(wdist), str(pdist)])
