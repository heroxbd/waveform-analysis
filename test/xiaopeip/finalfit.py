...
def xiaopeip_N(wave, spemean, peak_c, m_l, thres, mar_r, mar_l, l):
    ...
    if len(lowp) != 0:
        ...
        for j in lowp:
            ...
            tail = j+mar_r+1 if j+mar_r+1 <= l else l
            panel[head:tail] = 1
        ...
        if len(xuhao) != 0:
            ...
            if len(possible) != 0:
                ...
                b[:, 1] = np.inf
                mne = spemean[np.mod(nihep.reshape(len(nihep), 1) - possible.reshape(1, len(possible)), l)]
...
def main(fopt, fipt, single_pe_path):
    ...
    with h5py.File(fipt, 'r', libver='latest', swmr=True) as ipt:
        ...
        for i in range(l):
            ...
            pf = xiaopeip_N(wave, spemean, peak_c, m_l, thres, mar_r, mar_l, l)
