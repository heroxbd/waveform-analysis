...
PreData:=$(channelN:%=$(datfold)/$(set)/$(mode)/PreProcess/Pre_Channel%.h5)
...
$(tfold)/dist-$(set)/distr-$(1)-%.h5 : $(datfoldi)/$(set)/$(mode)/$(prefix)%.h5 $(tfold)/resu-$(set)/$(1)-%.h5 spe-$(set).h5
	@mkdir -p $$(dir $$@)
	python3 test_dist.py $$(word 2,$$^) --mod $(mode) --ref $$< $$(word 3,$$^) -o $$@ > $$@.log 2>&1
endef
$(foreach i,$(conv),$(eval $(call measure,$(i))))
$(tfold)/resu-$(set)/sub-%.h5 : $(tfold)/resu-$(set)/tot-%.h5 $(datfoldi)/$(set)/$(mode)/$(prefix)%.h5 spe-$(set).h5
	@mkdir -p $(dir $@)
	export OMP_NUM_THREADS=2 && python3 adjust.py $< $(word 2,$^) --mod $(mode) --ref $(word 3,$^) -o $@ > $@.log 2>&1
define fit
$(tfold)/resu-$(set)/tot-$(1).h5 : $(datfoldi)/$(set)/$(mode)/$(prefix)$(1).h5 spe-$(set).h5
	@mkdir -p $$(dir $$@)
	export OMP_NUM_THREADS=2 && python3 fit.py $$< --mod $(mode) --met $(method) --ref $$(wordlist 2,3,$$^) -N $(fragnum) -o $$@ > $$@.log 2>&1
endef

define nn
$(tfold)/resu-$(set)/tot-$(1).h5 : $(datfoldi)/$(set)/$(mode)/$(prefix)$(1).h5 spe-$(set).h5 $(Nets)
	@mkdir -p $$(dir $$@)
	python3 -u Prediction_Processing_Total.py $$< --mod $(mode) --met $(method) -o $$@ --ref $$(word 2,$$^) -N $(datfold)/$(set)/$(mode)/Nets -D 1 > $$(dir $$@)Analysis.log 2>&1
endef
...
$(datfold)/$(set)/$(mode)/$(prefix)Channel%/.Training_finished : $(datfold)/$(set)/$(mode)/PreProcess/Pre_Channel%.h5
	@mkdir -p $(dir $@)
	@mkdir -p $(datfold)/$(set)/$(mode)/Nets
	python3 -u Data_Processing.py $^ -n $* -B 64 -o $(datfold)/$(set)/$(mode)/Nets/Channel$*.torch_net $(dir $@) > $(dir $@)Train.log 2>&1
	@touch $@

$(PreData) : PreProcess

PreProcess : $(seq:%=$(datfold)/$(set)/$(mode)/$(prefix)%.h5) spe-$(set).h5
	@mkdir -p $(datfold)/$(set)/$(mode)/PreProcess
	python3 -u Data_Pre-Processing.py $(datfold)/$(set)/$(mode)/$(prefix) -o $(datfold)/$(set)/$(mode)/PreProcess/Pre_Channel --mod $(mode) --ref $(word $(words $^), $^) > $(datfold)/$(set)/$(mode)/PreProcess/PreProcess.log 2>&1

$(datfoldi)/$(set)/$(prefix)x.h5 : $(datfold)/$(set)/$(prefix)$(chunk).h5
	@mkdir -p $(dir $@)
	python3 cut_data.py $^ -o $@ -a -1 -b 100000

spe-$(set).h5 : $(datfold)/$(set)/Weight/$(prefix)$(word 1,$($(set)seq)).h5
