...
with PdfPages(args.opt) as opt:
    ...
    for i,pes in pedf:
        ...
        xmmse, xmmse_star, psy_star, nu_star, T_star, d_tot_i, d_max_i, expectx, expectx_star, num_i = mcp.fbmpr_poisson_deactive(wave_r, A, la, spe_pre[cid]['std'] ** 2,  (gsigma * factor / gmu) ** 2, factor, D=2*int(mu_t+3*np.sqrt(mu_t)),Q=np.max([10,maxQ]), stop=5,pmax=2*int(mu_t+3*np.sqrt(mu_t)),left=left_wave, right=right_wave, tlist=tlist, prior=prior)
