...
with h5py.File(args.ipt, 'r', libver='latest', swmr=True) as distfile:
    ...
    penum, c = np.unique(dt['NPE'], return_counts=True)
    pe_c = np.zeros((len(penum), 2)).astype(np.uint32)
    pe_dist = np.zeros((len(penum), 8))
    pe_c[:, 0] = penum
    pe_c[:, 1] = c
    for i in range(len(penum)):
        pe_dist[i, 0] = np.mean(dt['wdist'][dt['NPE'] == penum[i]])
        pe_dist[i, 1] = np.std(dt['wdist'][dt['NPE'] == penum[i]])
        pe_dist[i, 2] = np.percentile(dt['wdist'][dt['NPE'] == penum[i]], 5)
        pe_dist[i, 3] = np.percentile(dt['wdist'][dt['NPE'] == penum[i]], 95)
        pe_dist[i, 4] = np.mean(dt['chargediff'][dt['NPE'] == penum[i]])
        pe_dist[i, 5] = np.std(dt['chargediff'][dt['NPE'] == penum[i]])
        pe_dist[i, 6] = np.percentile(dt['chargediff'][dt['NPE'] == penum[i]], 5)
        pe_dist[i, 7] = np.percentile(dt['chargediff'][dt['NPE'] == penum[i]], 95)
with open(args.opt, 'w+') as csvf:
    ...
    str_pe_c = pe_c.astype(str)
    str_pe_dist = pe_dist.astype(str)
