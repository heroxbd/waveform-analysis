...
with h5py.File(args.ipt, 'r', libver='latest', swmr=True) as distfile:
    ...
    ppd1 = np.percentile(dt['chargediff'], 10)
    ppd2 = np.percentile(dt['chargediff'], 90)
    ...
    for i in tqdm(range(len(penum)), disable=args.pri):
        ...
        pe_dist[i, 2] = np.percentile(dt['wdist'][dt['TotalPEpos'] == penum[i]], 10)
        pe_dist[i, 3] = np.percentile(dt['wdist'][dt['TotalPEpos'] == penum[i]], 90)
        pe_dist[i, 4] = np.mean(dt['chargediff'][dt['TotalPEpos'] == penum[i]])
        pe_dist[i, 5] = np.std(dt['chargediff'][dt['TotalPEpos'] == penum[i]])
        pe_dist[i, 6] = np.percentile(dt['chargediff'][dt['TotalPEpos'] == penum[i]], 10)
        pe_dist[i, 7] = np.percentile(dt['chargediff'][dt['TotalPEpos'] == penum[i]], 90)
