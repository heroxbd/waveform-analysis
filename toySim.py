...
spe_pre = wff.read_model('../jinping/waveform/spe.h5')
spe = spe_pre[0]['spe']
...
def sampling(a0, a1, mu, tau, sigma):
    ...
    wdtp = np.dtype([('TriggerNo', np.uint32), ('ChannelID', np.uint32), ('Waveform', np.uint16, window)])
    waves = np.empty(a1 - a0).astype(wdtp)
    pan = np.arange(window)
    for i in range(a1 - a0):
        waves[i]['Waveform'] = np.around(np.sum([np.where(pan > sams[i][j, 0], wff.spe(pan - sams[i][j, 0], tau=p[0][0], sigma=p[0][1], A=p[0][2]) * sams[i][j, 1] / gmu, 0) for j in range(len(sams[i]))], axis=0) + np.random.uniform(0, spe_pre[0]['thres']/5, size=window)).astype(np.uint16)
...
with Pool(min(Ncpu, cpu_count())) as pool:
    result = pool.starmap(partial(sampling, mu=mu, tau=40., sigma=6.), slices)
...
with h5py.File(args.opt, 'w') as opt:
    ...
    opt.create_dataset('Readout/Waveform', data=waves, compression='gzip', compression_opts=4)
print(args.opt + ' saved')
