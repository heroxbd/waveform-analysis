...
def vcombine(A, cx, t):
    ...
    A_vec = (1 - frac)[:, :, None] * tf.gather(A, ti, batch_dims=1) + frac[
        :, :, None
    ] * tf.gather(A, ti + 1, batch_dims=1)
    c_vec = (1 - frac)[:, :, None] * tf.gather(cx, ti, batch_dims=1) + frac[
        :, :, None
    ] * tf.gather(cx, ti + 1, batch_dims=1)
    return A_vec, c_vec
...
def batch(A, cx, index, tq, s, z):
    ...
    home_s = np.array(
        [
            np.interp(_is, xp=_xp[:_lt], fp=fp[:_lt])
    ...
    for i, (t, step, home, wander, wt, accept, acct) in enumerate(
        ...
        lc0 = tf.reduce_sum(tf.where(sel, lc(rt - t0[:, None]), 0), axis=1)
        lc1 = tf.reduce_sum(tf.where(sel, lc(rt - nt0[:, None]), 0), axis=1)
        ...
        Δν[e_pm] += step[e_pm] * (
            log_mu[e_pm]
            - np.log(tq["q_s"][e_pm, np.array(loc[e_pm, 1], dtype=np.int32) + 1])
            - np.log(NPE[e_pm])
        )
        loc[e_annihilate, 1] = l_t
        NPE[e_create] -= 1
...
for part in range(l_e // args.size + 1):
    ...
    if l_part:
        ...
        (flip, s0_history, t0_history, Δν_history, annihilations, creations) = batch(
            tf.constant(A_slice, dtype=tf.float32),
